...
function replaceImg(htmlContent,rootUrl)
{
    return htmlContent.replace(/<img\n?(.*?)src="(.*?)"\n?(.*?)>/gi, function (
        ...
        return str.replace(p2, rootUrl + p2 + `${hexo.config.cdn.webImage? '-web' : ''}?v=${Math.random()}`);
      });
}
/**
 *
...
function replaceCss(htmlContent,rootUrl)
{
    return htmlContent.replace(
        ...
        function (str, p1, p2) {
            ...
            return str.replace(p1, rootUrl + p1 + `${hexo.config.cdn.webImage? '-web' : ''}?v=${Math.random()}`);
        }
    );
}
