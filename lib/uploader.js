...
if(hexo.config.qiniu.env){
    var qnSec = hexo.config.qiniu;
    if(qnSec.secret_key){
        hexo.config.qiniu.secret_key=process.env[qnSec.secret_key]
    }
    if(qnSec.access_key){
        console.log('process.env[qnSec.access_key]:',process.env[qnSec.access_key]);
        hexo.config.qiniu.access_key=process.env[qnSec.access_key];
    }
}
// 七牛的配置组
...
var upload = ()=>{
    ...
    return sync.call(this,'public').then(()=>{
        uploadFileArr.forEach(async item=>{
            await check_upload(item.fname, item.name)
        })
    });
}
