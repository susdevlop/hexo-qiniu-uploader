...
if(!config.urlPrefix){
    throw new Error('must be provide a urlPrefix');
    return false
} else {
    config.url_Prefix = config.urlPrefix;
}
log.i('-----------------------------------------------------------');
log.i('qiniu url:   ', config.url_Prefix);
...
function uploadFile(key, localFile) {
    ...
    qiniu.io.putFile(uptoken, key, localFile, extra, function(err, ret) {
        if(!err) {
            // 上传成功， 处理返回值
            log.i('上传成功',ret.hash, ret.key, ret.persistentId);
        } else {
            // 上传失败， 处理返回代码
            log.e('上传失败',err);
        }
